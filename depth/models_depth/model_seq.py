...
from vpd.models import TextWrapper, UNetWrapper, TextAdapterDepth

class VPDDepthEncoderSeq(nn.Module):
    def __init__(self, out_dim=1024, ldm_prior=[320, 640, 1280+1280], sd_path=None, text_dim=768,
                 dataset='nyu', use_adapter=True
                 ):
        super().__init__()
        ...
        if dataset == 'nyu':
            if use_adapter:
                self.text_encoder = TextAdapterDepth(text_dim=text_dim)
            else:
                self.text_encoder = TextWrapper(text_dim=text_dim)
        ...
        feat_dim = [320, 640, 1280, 1280]
        self.conv_agg = nn.ModuleList([
            nn.Conv2d(feat_dim[i] * 4, feat_dim[i], 1) for i in range(4)
        ])
    ...
    def forward(self, x, class_ids=None):
        ...
        c_crossattn = self.text_encoder(latents, class_embeddings, self.gamma)  # NOTE: here the c_crossattn should be expand_dim as latents

        outs_agg = []
        for t in [0, 250, 500, 750]:
            noise = torch.randn_like(latents)
            noisy_latents = latents * self.sd_model.sqrt_alphas_cumprod[t] + noise * self.sd_model.sqrt_one_minus_alphas_cumprod[t]

            timesteps = torch.ones((x.shape[0],), device=x.device).long() * t
            outs = self.unet(noisy_latents, timesteps, c_crossattn=[c_crossattn])
            if not outs_agg:
                outs_agg = outs
            else:
                for i in range(len(outs)):
                    outs_agg[i] = torch.cat([outs_agg[i], outs[i]], dim=1)
        
        for i in range(len(outs_agg)):
            # print(f"{i}th outs_agg shape: {outs_agg[i].shape}")
            outs_agg[i] = self.conv_agg[i](outs_agg[i])
        
        feats = [outs_agg[0], outs_agg[1], torch.cat([outs_agg[2], F.interpolate(outs_agg[3], scale_factor=2)], dim=1)]
        x = torch.cat([self.layer1(feats[0]), self.layer2(feats[1]), feats[2]], dim=1)
        return self.out_layer(x)

class VPDDepthSeq(nn.Module):
    def __init__(self, args=None):
        ...
        if args.dataset == 'nyudepthv2':
            self.encoder = VPDDepthEncoderSeq(out_dim=channels_in, dataset='nyu', use_adapter=args.use_adapter)
        else:
            raise NotImplementedError
